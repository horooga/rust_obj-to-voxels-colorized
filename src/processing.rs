use crate::misc::{Brick, Vertex, index, process_vertices_faces};
use crate::tribox::*;
use std::{
    collections::HashMap,
    fs::File,
...
pub fn obj2voxel(
    ...
    let mut voxels: Vec<u16> = vec![0; size.pow(3)];
...
pub fn merge_voxels(
            ...
            for y in 0..size {
                ...
                while x < size {
                    ...
                    bricks.push(Brick {
                        ...
                        max: [
                            (x.saturating_sub(1) as f32 + 0.5) * voxel_size,
                            (y as f32 + 0.5) * voxel_size,
                            (z as f32 + 0.5) * voxel_size,
                        ],
            ...
            for x in 0..size {
                ...
                while y < size {
                    ...
                    bricks.push(Brick {
                        ...
                        max: [
                            (x as f32 + 0.5) * voxel_size,
                            (y.saturating_sub(1) as f32 + 0.5) * voxel_size,
                            (z as f32 + 0.5) * voxel_size,
                        ],
    ...
    println!("Merged {} bricks from {} voxels", bricks.len(), voxel_count);
...
fn load_obj(
    path: &str,
    mtl_file_path: Option<&str>,
) -> std::io::Result<(Vec<[f32; 3]>, Vec<Vec<usize>>, Vec<u16>, Vec<String>)> {
    let file = File::open(path).expect("Failed to open {path}");
    let reader = BufReader::new(file);

    let mut vertices: Vec<[f32; 3]> = Vec::new();
    let mut faces: Vec<Vec<usize>> = Vec::new();
    let mut curr_color_idx: u16 = 65535;
    let mut face_colors_idxs: Vec<u16> = Vec::new();
    ...
    for line_res in reader.lines() {
        ...
        if line.starts_with("v ") {
            ...
            if parts.len() >= 4 {
                ...
                let z: f32 = parts[1].parse().unwrap_or(0.0);
                vertices.push([x, y, z]);
            }
            ...
            for (idx, i) in colors.iter().enumerate() {
                if color == *i {
                    curr_color_idx = idx as u16;
                    found = true;
                    break;
                }
            }
            if !found {
                eprintln!("Color used in .obj file but not declared in .mtl file");
            }
        }
    }
        ...
        if let Some(string) = parts.next() {
            if string == "newmtl" {
                if let Some(color) = current_color.as_ref() {
                    if colors.len() > 65535 {
                        eprintln!("Max amount of colors reached (65535 for storing as u16)");
                    }
...
pub fn save_as_obj(
    ...
    for face in faces {
        let brick_color_idx = bricks[face_count / 6].color_idx as usize;
